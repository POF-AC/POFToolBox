...
/**
 * This class implements the angular power distribution of an area source over thata z and theta phi.
 * 
 * @author Thomas Becker
 *
 */
public class ModalDistribution2DArea implements Function3D 
{
	/**
	 ...
	 */
	private AngularTransformationBeta2TP2Area m_atB2TP;
	...
	/**
	 * This object describes the angular power distribution of theta z
	 */
	private LightSourceAngularDistribution m_ls;
	 ...
	 * @param a_lsenum					This object describes the angular power distribution of theta z
	 */
	public ModalDistribution2DArea( 
			FiberProps a_fp, 
			double a_dblMinAngle,
			double a_dblMaxAngle,
			double a_dblPT,
			boolean a_bNeglectRefractedModes,
			LightSourceAngularEnum a_lsenum
	)
	{
		...
		m_atB2TP = new AngularTransformationBeta2TP2Area(fp);
		m_bNeglectRefractedModes = a_bNeglectRefractedModes;
		m_ls = new LightSourceAngularDistribution(a_lsenum, a_fp, a_dblPT);
	}
	...
	public double getValue(double a_dblTZ, double a_dblTP) 
	{
    	if(a_dblTZ <= m_dblMaxAngle && a_dblTZ >= m_dblMinAngle)
    	{
	        ...
	        double dblErg =  
	        		(2.0/Math.PI)*m_ls.getValue(a_dblTZ)
	        		*
